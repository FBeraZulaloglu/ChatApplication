...
const allRooms = document.getElementById("chat-rooms")
...
socket.on('newChatRoom', ({ rooms }) => {
    displayChatRooms(rooms)
})

// Get new chat room
socket.on('displayChatRoom', ({ messages }) => {
    if (isSelectedRoom)
        displayChatRoomMessages(messages)
...
function displayMessage(message) {
    ...
    if (username != message.username) {
        ...
        div.innerHTML = `<img src="img/defaultAvatar.png" alt="Avatar">
    <p><label class="sender-text">${message.username} </label><label class="sender-text">${message.time}</label><br>
    ${message.text}</p>`
    } else {
        div.classList.add("container")
        div.classList.add("darker")
        div.innerHTML = `<img src="img/defaultAvatar.png" alt="Avatar" class="right">
    <p><label class="sender-text">${message.username} </label><label class="sender-text">${message.time}</label><br>
    ${message.text}</p>`
    }
...
function displayMessages(messages) {
    ...
    ${message.text}</p>`
            ...
            div.classList.add("container")
            div.innerHTML = `<img src="img/defaultAvatar.png" alt="Avatar">
    <p><label class="sender-text">${message.target} </label><label class="sender-text">${message.time}</label><br>
    ${message.text}</p>`
...
function displayUsers(users) {
    ...
    for (const user of users) {
        if (currentUser.innerText == user.username) continue // do not add user itself
        const div = document.createElement("div")
        ...
        document.getElementById(user.id).onclick = () => {
            ...
            document.getElementById("chat-source").innerHTML = "User : "+user.username
        };
    }
}
...
function displayChatRooms(rooms) {
    ...
    for (const room of rooms) {
        ...
        document.getElementById(room.roomname).onclick = () => {
            ...
            socket.emit("joinChatRoom", selectedRoomName)
            console.log("selected room: " + room.roomname)
            document.getElementById("chat-source").innerHTML = "Room : "+room.roomname
        }
    }
}
...
chatForm.addEventListener('submit', (e) => {
    ...
    if (isSelectedTargetClient) {
        if (isSelectedFile) {
            ...
            console.log("Message is not file")
            // And display it
            displayMessage({
                username,
                text: msg,
        ...
        if (isSelectedFile) {
            // will be updated
            sendFile("displayChatRoom", selectedFile, username, selectedRoomName)
            isSelectedFile = false
        } else {
            // Send message for room
            socket.emit("displayChatRoom", { selectedRoomName, username, msg, type: "text" })
        }
